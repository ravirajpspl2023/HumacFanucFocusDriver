...
class FocasDriverBase(object):
    ...
    def __init__(self, filename, extradlls=[], timeout=10):
        ...
        for extradll in extradlls:
            ...
            self.extradlls.append(ctypes.CDLL(extradll))
         
        # dllfile = find_library(filename)
        logging.info(f"Loading FOCAS library from {filename}")
        self.dll = ctypes.CDLL(filename)
        logging.info("FOCAS library loaded successfully")
        self._poll_methods = []
